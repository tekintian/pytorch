...
MIN_APPROVALS = 3
SINCE = "90 days ago"
MERGE_RULES = Path(__file__).parent / "merge_rules.yaml"
...
def fetch_metamates() -> set[str]:
    ...
    res = subprocess.run(
        ["gh", "api", "orgs/pytorch/teams/metamates/members",
         "--paginate", "--jq", ".[].login"],
...
def update_approved_by(content: str, new_members: list[str]) -> str:
    ...
    if start_idx is None or end_idx is None:
        print(f"ERROR: Could not find approved_by in '{METAMATES_RULE}' rule",
              file=sys.stderr)
        sys.exit(1)
...
def main() -> None:
    ...
    if not any(r.get("name") == METAMATES_RULE for r in rules):
        print(f"ERROR: No rule named '{METAMATES_RULE}' in {MERGE_RULES}",
              file=sys.stderr)
        sys.exit(1)
